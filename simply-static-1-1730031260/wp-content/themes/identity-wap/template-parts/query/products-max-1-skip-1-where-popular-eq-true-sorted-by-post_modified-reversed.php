...
$args = apply_filters('udesly/posts/products-max-1-skip-1-where-popular-eq-true-sorted-by-post_modified-reversed', $args);
                        ...
                        <?php if ( $query->have_posts() ) : ?><div role="list" class="showcase-products single-column w-dyn-items">
                          <?php while ($query->have_posts()) : $query->the_post(); global $post, $product, $variant; ?><div role="listitem" class="w-dyn-item">
                            <a href="<?php the_permalink() ?>" class="product-card w-inline-block">
                              ...
                              <div class="card-info">
                                <div id="w-node-f120e14c-d451-f10b-57f6-880518d62e85-aa2582a9" class="product-card-top">
                                  <div class="product-card-info">
                                    <div class="product-tags">
                                      <?php if (udesly_get_custom_post_field( $post->ID, "new-product", "Bool" )) : ?><div class="new-tag">
                                        <div class="label-text">New</div>
                                      </div><?php endif  ?>
                                      <?php if (udesly_get_custom_post_field( $post->ID, "discount-percentage", "PlainText" )) : ?><div class="discount-label"><img src="<?php echo udesly_get_image(_u('i152f9a01', 'img'))->src ?>" loading="lazy" id="w-node-f120e14c-d451-f10b-57f6-880518d62e8c-aa2582a9" alt="<?php echo udesly_get_image(_u('i152f9a01', 'img'))->alt ?>" class="tag-icon" data-img="i152f9a01" srcset="<?php echo udesly_get_image(_u('i152f9a01', 'img'))->srcset ?>">
